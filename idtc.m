...
q = zeros(numsim,1);
q_lb     = zeros(numsim,1); % lower bound from dirty sector
q_ub     = zeros(numsim,1); % upper bound from adaptive sector
q_rhs_ad = zeros(numsim,1); % RHS of adaptive inequality
...
if s_c(1) == 1 || s_c(1) == 0
    ...
    q_rhs_ad(1) = (eta_a / eta_c) * Omega(1)^(1 - epsilon) * ...
        ( ((1 + gamma * eta_c * s_c(1)) * Ac0) / ...
          ((1 + gamma * eta_a * s_a(1)) * Aa0) )^(phi + 1) * ...
        (Aa0 / Ac0)^(-phi);

    q_ub(1) = idr(1) * (1 - (q_rhs_ad(1))^(1 / epsilon));

    q(1) = max([q_lb(1), q_ub(1), 0]);
end
...
for t = 2:numsim
    if s_c(t) == 1 || s_c(t) == 0
        ...
        q_rhs_ad(t) =  (eta_a / eta_c) * Omega(t)^(1 - epsilon) * ...
            ( ((1 + gamma * eta_a * s_a(t))) / ...
              ((1 + gamma * eta_c * s_c(t))) )^(-phi - 1) * ...
            (A_a(t-1) / A_c(t-1))^(-phi);

        q_ub(t) = idr(t) * ((q_rhs_ad(t))^(1 / epsilon) - 1);

        q(t) = max([q_lb(t), q_ub(t)]);
    end
end

% Optimizing scientist allocations
for t = 2:numsim
    pi_cd(t) = (1 + q(t) * (1/Omega(t))) * (1 + tau_d(t))^epsilon * (eta_c / eta_d) * ...
            ((1 + gamma * eta_c * s_c(t-1)) / (1 + gamma * eta_d * s_d(t-1)))^(-phi - 1) * ...
            (A_c(t-1) / A_d(t-1))^(-phi);

    % Calculate expected profit ratio vs adaptive
    pi_ca(t) = (1 + q(t) * idr(t))^epsilon * (eta_c / eta_a) * ...
            Omega(t)^( -(1 - epsilon) ) * ...
            (((1 + gamma * eta_c * s_c(t-1)) * A_c(t-1)) / ((1 + gamma * eta_a * s_a(t-1)) * A_a(t-1)))^(-phi - 1) * ...
            (A_c(t-1) / A_a(t-1));

    % If clean dominates both: reallocate all scientists
    if pi_cd(t) > 1 && pi_ca(t) > 1
        s_c(t) = 1;
        s_d(t) = 0;
        s_a(t) = 0;
    end
    
    % Sectoral persistance
    if s_c(t-1) == 1
        s_c(t) = 1;
        s_d(t) = 0;
        s_a(t) = 0;
